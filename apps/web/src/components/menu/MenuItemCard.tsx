...
export const MenuItemCard = memo(function MenuItemCard({ item, promoPreview, quickAddEnabled = false }: MenuItemCardProps) {
  ...
  // Quick-add: available, no modifiers, quickAddEnabled
  const showQuickAdd = quickAddEnabled && !item.has_modifiers && item.is_available
  ...
  return (
    <Card className={cn(
      ...
      {isUnavailable ? (
        ...
        // Quick-add layout: Link wraps image+text, button is a sibling
        <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4">
          <Link
            href={itemHref}
            aria-label={detailActionLabel}
            data-testid="menu-item-primary-link"
            className="flex flex-1 items-center gap-3 sm:gap-4 min-w-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
          >
            {imageBlock}
            {textBlock}
          </Link>
          <div className="hidden sm:flex flex-col items-end justify-center flex-shrink-0">
            <button
              ...
              className="inline-flex items-center rounded-full border border-primary/40 bg-primary/10 px-3 py-1 text-xs font-medium uppercase tracking-wider text-primary transition-colors hover:bg-primary hover:text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
            >
              Add +
            </button>
          </div>
        ...
        // Default: entire card is a Link (existing behavior)
        <Link
          href={itemHref}
          aria-label={detailActionLabel}
          data-testid="menu-item-primary-link"
